...
if (isset($header["Content-Type"]) && $header["Content-Type"] == "application/json-charset=utf-8") {
    ...
    $listFieldNamesLog = Logging::defaultLogging()->getListFieldNames();
    ...
    if (in_array($fieldSearch, $listFieldNamesLog)){
        ...
        if ($sqlData->getConnectionErreur() == 0) {
            ...
            $resultRequestGetLogs = $sqlData->get_logs_with_attribute("Users", $fieldSearch, $stringToSearch);

            $sqlData->close_connexion_to_db();

            //on regarde si une erreur est survenue au cours du script
            if ($resultRequestGetLogs["error"] == 0){
                $listeResultParams["result"] = $resultRequestGetLogs["result"];
            }
            else{
                $errorMessage = $resultRequestGetLogs["errorMessage"];
            ...
            $loggerFile->error($userId, getTodayDate(), $_SERVER['REMOTE_ADDR'], "Erreur script_get_logs_with_attribute|Erreur:{$sqlData->getConnectionErreurMessage()}");
            $listeResultParams["error"] = 1;
        ...
        $loggerFile->error($userId, getTodayDate(), $_SERVER['REMOTE_ADDR'], "Erreur script_get_logs_with_attribute|Erreur:{$errorMessage}");
        $listeResultParams["error"] = 1;
        $listeResultParams["errorMessage"] =  $errorMessage;
    }
